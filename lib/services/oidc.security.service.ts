...
export class OidcSecurityService {
    ...
    authorize(urlHandler?: (url: string) => any) {
        ...
        let url = '';
        // Code Flow
        console.log('Response Type: ', this.configurationProvider.openIDConfiguration.response_type);
        if (this.configurationProvider.openIDConfiguration.response_type === 'code') {
            ...
            this.loggerService.logDebug('Code Verifier: ' + code_verifier);
    ...
    authorizedCallbackWithCode(urlToCheck: string) {
        console.log('Authorization callback with code wso2');
    ...
    requestTokensWithCodeProcedure$(code: string, state: string, session_state: string | null): Observable<void> {
        ...
        if (this.oidcSecurityCommon.silentRenewRunning === 'running') {
            console.log('Silent renew request condition');
